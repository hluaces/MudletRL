...
bool TMap::restore(QString location)
{
    ...
    if( ! canRestore || entries.size() == 0 )
    {
        ...
        if( mpHost->mUrl.toLower().contains( "achaea.com" )
            ...
            // Zoilder 06/04/2015: Añado reinos de leyenda para que autodescargue su mapa.
            || mpHost->mUrl.toLower().contains( "reinosdeleyenda.es")
            || mpHost->mUrl.toLower().contains( "rlmud.org"))
        {
            ...
            msgBox.exec();
            init( mpHost );
            // Zoilder 07/04/2015: Si indica que descargue el mapa, devuelve false,
            //                     para que así se sepa esto y por ejemplo,
            //                     no muestre el dock del mapa hasta que se descargue.
