...
// Zoilder 31/03/2015: Incluyo la clase TTextEdit para permitir limpiar la pantalla si se indica el código ansi adecuado.
#include "TTextEdit.h"
    ...
    while( true )
    {
        ...
        if( gotHeader )
        {
            while( msPos < msLength )
            {
                ...
                // Zoilder 08/04/2015: Sólo cuando no se indique que inactive la negociación de MXP.
                ...
                else if( ch2 == 'm' )
                {
                    ...
                    for( int i=1; i<codeRet+1; i++ )
                    {
                        ...
                        case 1:
                            mBold = true;
                            // Zoilder 01/04/2015: Como en CMud: Bold = Bold - White.
                            /*if(fgColorR == mFgColorR){
                                fgColorR = mWhiteR;
                                fgColorLightR = mLightWhiteR;
                            }
                            if(fgColorG == mFgColorG){
                                fgColorG = mWhiteG;
                                fgColorLightG = mLightWhiteG;
                            }
                            if(fgColorB == mFgColorB){
                                fgColorB = mWhiteB;
                                fgColorLightB = mLightWhiteB;
                            }
                            mIsDefaultColor = false;*/
            ...
            if( ch == '>' )
            {
                ...
                if( ( openT > 0 ) && ( closeT == openT ) )
                {
                    ...
                    if( _tn == "VERSION" )
                    {
                        ...
                        // Zoilder 18/05/2015: Cambiada versión del cliente que se informa al Mud. Hay suficientes
                        //                     cambios para considerar la versión como propia del RLII.
                        mpHost->sendRaw( QString("\n\x1b[1z<VERSION MXP=1.0 CLIENT=Mudlet VERSION=3.0.1-RLII>\n") );
                    }
                    if( _tn == "BR" )
                    {
                        ch = '\n';
...
//            }
        ...
        if( ch != '\t' )
        {
            // Zoilder 15/03/2015: Procesa el carácter leído y si es el inicio de una cadena UTF, leerá todos los bytes
            //                     necesarios.
            //mMudLine.append( ch );
            ...
            if(ch<0){
                ...
                // Lee el número de bytes que conforman el carácter.
                while(msPos + totalBytes < msLength && (s[msPos + totalBytes] & 0xc0) == 0x80)
                    totalBytes++;

                // Inicia el carácter.
                char *plainText = (char *)malloc(totalBytes + 1);
                ...
                *plainText = 0;
                msPos--;
                QString actual = QString::fromUtf8(plainText - totalBytes);
                mMudLine.append(actual.at(0));
            }
            // Carácter normal. Se lee desde el Latín.
            else
                mMudLine.append(QChar::fromLatin1(ch));
...
inline int TBuffer::wrap( int startLine )
{
    ...
    for( int i=startLine; i<static_cast<int>(buffer.size()); i++ )
    {
        ...
        for( int i2=0; i2<static_cast<int>(buffer[i].size());  )
        {
            ...
            // Zoilder 18/03/2015: Recorre hasta la posición calculada - 1, si no, cada línea
            // tendrá un carácter más del indicado.
