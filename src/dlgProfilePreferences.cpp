...
// Zoilder 07/04/2015: Include para permitir mostrar MessageBoxes.
#include <QMessageBox>
...
, mpHost( pH )
{
    ...
    // Zoilder 06/04/2015: Añado nuestro mud para permitir autodescargar el mapa.
    if( pH->mUrl.toLower().contains("achaea.com") || pH->mUrl.toLower().contains("aetolia.com") || pH->mUrl.toLower().contains("imperian.com") || pH->mUrl.toLower().contains("midkemiaonline.com") || pH->mUrl.toLower().contains("lusternia.com") ||
        pH->mUrl.toLower().contains("reinosdeleyenda.es") || pH->mUrl.toLower().contains("rlmud.org") )
    {
    ...
    // Zoilder 18/03/2015: Se usa clicked, de lo contrario se ejecuta 2 veces en ciertas ocasiones.
    ...
    if( pHost )
    {
        ...
        // Zoilder 18/03/2015: UTF-8 setting.
        mUseUTF8->setChecked( pHost->mTelnet.mUseUTF8 );
...
void dlgProfilePreferences::downloadMap()
{
    ...
    // Zoilder 07/04/2015: Ya hay un mapa asociado, simplemente lo descarga y sobreescribe.
    //                     Inicialmente no usaba else, lo que hacía cosas raras al pulsar el botón de descargar (Abría 2 ventanas de descargar y se liaba)
    else{
        // Zoilder 07/04/2015: Se está descargando el mapa.
        if(mpHost->mpMap->mpMapper->mpProgressDialog &&
                mpHost->mpMap->mpMapper->mpProgressDialog->isVisible()){
            QMessageBox msgBox;
...
void dlgProfilePreferences::slot_save_and_exit()
{
  ...
  qDebug()<<"Left border width:"<<pHost->mBorderLeftWidth<<" right:"<<pHost->mBorderRightWidth;
    ...
    // Zoilder 18/03/2015: UTF-8 setting.
    pHost->mTelnet.mUseUTF8 = mUseUTF8->isChecked();
    if(pHost->mTelnet.mUseUTF8 && pHost->mTelnet.encoding != "UTF-8" ||
       !pHost->mTelnet.mUseUTF8 && pHost->mTelnet.encoding != "ISO-8859-1")
            pHost->mTelnet.encodingChanged( mUseUTF8->isChecked() ? "UTF-8" : "ISO-8859-1" );
