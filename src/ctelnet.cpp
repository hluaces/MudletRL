...
// Zoilder 29/03/2015: Se incluye la clase de campos de texto para poder así enviar al Mud en la configuración inicial el número de columnas.
#include "TTextEdit.h"
...
// Zoilder 18/03/2015: Configuración UTF-8. Por defecto usará ISO-8859-1, hasta que se active la opción en settings.
, mUseUTF8( false )
// Zoilder 31/03/2015: Control del eco de los comandos. Por defecto siempre habrá eco, salvo cuando la Mudlib indique lo contrario.
, mCommandEcho( true )
{
    ...
    // Zoilder 31/03/2015: Establece el encoding inicial adecuado según el constructor de mUseUTF8.
    encoding = mUseUTF8 ? "UTF-8" : "ISO-8859-1";
...
void cTelnet::reset ()
{
    ...
    command = "";
    mMudData = "";
    // Zoilder 08/04/2015: Resetea las nuevas variables añadidas.
    enableGMCP = false;
    mCommandEcho = true;
}
...
{
    qDebug() << "cTelnet::encodingChanged() called with " << encoding << " encoding!";
    ...
    // Zoilder 18/03/2015: Notifica al Mud el encoding que debe usar.
    if( enableGMCP )
        resetGMCP(false);
}
...
{
    // Zoilder 07/04/2015: Esto provoca errores al cerrar el cliente teniendo el mapa abierto, etc. Lo quito por ahora.
    //postData();
    TEvent me;
    me.mArgumentList.append( "sysDisconnectionEvent" );
    me.mArgumentTypeList.append( ARGUMENT_TYPE_STRING );
...
void cTelnet::replyFinished( QNetworkReply * reply )
{
    ...
    // Zoilder 01/04/2015: Se controla si se ejecutó el proceso sin errores.
    if(reply->error() == QNetworkReply::NoError){
        ...
        /* Zoilder 30/03/2015: Desactivado. El paquete se almacena en installpackage,
                               pues el nombre puede variar (leyéndolo del config.lua)
...
void cTelnet::processTelnetCommand( const string & command )
{
  ...
  switch( ch )
  {
      ...
      case TN_WILL:
      {
          ...
          if( option == GMCP )
          {
              // Zoilder 18/03/2015: Reactivo para que no inicie la comunicación GMCP cuando no está
              // activa en Settings.
              if( !mpHost->mEnableGMCP ) break;
              ...
              // Notifica al mud de la información de GMCP inicial.
              resetGMCP(true);
              break;
          }
          ...
          else
          {
              if( !hisOptionState[idxOption] )
              {
                  ...
                  // Zoilder 31/03/2015: Adaptado para que se controle cuando no debe realizar el eco de los comandos introducidos.
                   ...
                   // Zoilder 31/03/2015: No debe mostrar el eco. Fluffos manda esto cuando
                   //                     se realiza un input_to con el flag de no eco activo.
                   else if( option == OPT_ECHO ){
                       sendTelnetOption( TN_DO, option );
                       setCommandEcho(false);
                       qDebug() << "Echo is disabled!";
                   }
      ...
      case TN_WONT:
      {
          ...
          else
          {
              ...
              if( hisOptionState[idxOption] || ( heAnnouncedState[idxOption] ) )
              {
                  ...
                  // Zoilder 31/03/2015: Debe mostrar el eco. Fluffos manda esta secuencia
                  //                     cuando el usuario está dentro de un input_to
                  //                     con el flag no_echo activo, justo antes de ejecutar
                  //                     el texto recibido para dicho input_to.
                  if( option == OPT_ECHO ){
                    setCommandEcho(true);
                    qDebug() << "Echo is enabled !";
                  }
              }
              heAnnouncedState[idxOption] = true;
          }
      ...
      qDebug() << "telnet: server wants us to enable option:"<< (quint8)command[2];
          ...
          {
              // Zoilder 08/04/2015: Sólo activa el MXP si no se indica lo contrario en settings.
            if( ! mpHost->mFORCE_MXP_NEGOTIATION_OFF ){
                sendTelnetOption( TN_WILL, 91 );
                mpHost->mpConsole->print("\n<MXP support enabled>\n");
                // Zoilder 20/03/2015: Se inactiva el mxp en el buffer, de lo contrario, la primera vez que se abre
                // el cliente los mensajes se mostrarían como si el cliente no soportara MXP, lo que haría
                // que se comiera el texto entre < >, etc.
                // Una vez que el cliente ya está abierto y ha mostrado los primeros mensajes, se
                // activará auto cuando el mud envíe texto en MXP.
                // Se hace así, porque aunque se le manda en este punto la notificación al driver
                // para activar el MXP, en realidad el driver no lo procesa hasta que ha enviado los
                // mensajes iniciales en el login.
                mpHost->mpConsole->buffer.mMXP = false;
            }
            break;
          }
      ...
      case TN_SB:
      {
          ...
          if( option == static_cast<char>(201) )
          {
              //QString _m = command.c_str();
              //QString _m(command.c_str());
              // Zoilder 14/03/2015: Se pasa a UTF-8 el contenido válido GMCP recibido, sin basura.
              std::string texto_gmcp = command.substr(3, command.size() - 5);
              QString _m = encodeUTF8(texto_gmcp);

              if( command.size() < 6 ) return;
              // Zoilder 13/03/2015: Los datos GMCP se reciben en UTF, se eliminan los datos 'morralla' del final
              // usando la propia QString formada ya en UTF-8, en lugar de usar el propio comando recibido,
              // pues de este modo no sustituye lo necesario.
              //_m = _m.mid( 3, command.size()-5 );
//              _m = _m.mid( 3, _m.size()-5 );
              setGMCPVariables( _m );
              return;
          }
...
// N/U:        single = false;
    ...
    if( msg.startsWith( "Client.GUI" ) )
    {
        ...
        if( mpHost->mServerGUI_Package_version != newVersion )
        {
            // Zoilder 30/03/2015: Cambiado el orden de los parámetros, que estaba al revés y añadido salto de línea.
            QString _smsg = QString("<The server wants to upgrade the GUI to new version '%1'. Uninstalling old version '%2'>\n").arg(newVersion).arg(mpHost->mServerGUI_Package_version);
            mpHost->mpConsole->print(_smsg.toLatin1().data());
            mpHost->uninstallPackage( mpHost->mServerGUI_Package_name, 0);
            mpHost->mServerGUI_Package_version = newVersion;
        }
        ...
        mpHost->mpConsole->print(packageName);
        mpHost->mpConsole->print("')>\n");
        // Zoilder 30/03/2015: Puede que el paquete esté instalado con un nombre distinto al de su fichero,
        //                     así que comprueba de las 2 formas.
        if( mpHost->mInstalledPackages.contains( packageName ) ||
            mpHost->mInstalledPackages.contains(mpHost->mServerGUI_Package_name))
        {
            mpHost->mpConsole->print("<package is already installed>\n");
            return;
        }
                    ...
                    if( ( mMCCP_version_1 || mMCCP_version_2 ) && ( ! mNeedDecompression ) )
                    {
                        ...
                        if( (_ch == OPT_COMPRESS ) || (_ch == OPT_COMPRESS2 ) )
                        {
                            ...
                            if( _compress )
                            {
                                ...
                                // Zoilder 13/03/2015: Codifica el texto obtenido en UTF-8. Esto permite conectar a Muds
                                //                     que usan ISO-8859-1 por ejemplo, y que todo lo que envíe dicho Mud,
                                //                     se convierta a UTF-8 para que el Mudlet lo procese correctamente.
                                cleandata = encodeUTF8(cleandata).toUtf8().data();
                                gotRest( cleandata );
                                ...
                                // Zoilder 30/03/2015: Corregido para que si descomprime el buffer, reinicie 'i',
                                //                     para que de este modo procese correctamente todo el buffer descomprimido.
                                //                     De no hacerse así, en ciertas ocasiones 'se come' el texto inicial
                                //                     del buffer.
                                //                     Si tras iniciar la compresión no hay más texto, entonces incrementa i
                                //                     en 2 (Los caracteres restantes de la secuencia).
                                //                     Se reinicia además el comando actual, pues debe comenzar a contar
                                //                     con el contenido descomprimido, si no, no ejecutaría por ejemplo
                                //                     un comando GMCP si era el primer contenido enviado tras la señal
                                //                     de inicio de compresión MCCP.
                                if( restLength > 0 )
                                {
                                ...
                                iac = false;
                                insb = false;
                                // Zoilder 30/03/2015: Reinicia el comando actual, pues debe comenzar a contar desde 0,
                                //                      pues contiene la secuencia de MCCP.
                                command = "";
                                ////////////////
                                goto MAIN_LOOP_END;
                            }
...
MAIN_LOOP_END: ;
        if( recvdGA )
        {
            if( ! mFORCE_GA_OFF ) //FIXME: wird noch nicht richtig initialisiert
            {
                ...
                //                     que usan ISO-8859-1 por ejemplo, y que todo lo que envíe dicho Mud,
                //                     se convierta a UTF-8 para que el Mudlet lo procese correctamente.
                //cleandata = encodeUTF8(cleandata).toLatin1().data();
                cleandata = encodeUTF8(cleandata).toUtf8().data();
    ...
    if( cleandata.size() > 0 )
    {
        ...
        //                     que usan ISO-8859-1 por ejemplo, y que todo lo que envíe dicho Mud,
        //                     se convierta a UTF-8 para que el Mudlet lo procese correctamente.
        //cleandata = QString::fromUtf8(cleandata.data()).toLatin1().data();
        cleandata = encodeUTF8(cleandata).toUtf8().data();
       gotRest( cleandata );
    }
    mpHost->mpConsole->finalize();
...
// Zoilder 18/03/2015: Función por la que pasará todo el flujo Mud->Mudlet y convierte
//                     los datos a UTF-8 desde el encoding actual (ISO o UTF)
QString cTelnet::encodeUTF8(std::string &s){
    return incomingDataDecoder->toUnicode(s.c_str());
}

// Zoilder 31/03/2015: Funciones para controlar si hay eco de los comandos.
void cTelnet::setCommandEcho(bool echo){
    mCommandEcho = echo;
}
bool cTelnet::getCommandEcho(){
    return mCommandEcho;
}

// Zoilder 05/06/2015: Función para solicitar que envíe al mud la información de GMCP (o la parte de la configuración)
void cTelnet::resetGMCP(bool complete){
    string _h;

    if( !mpHost->mEnableGMCP ) return;
    ...
    // Zoilder 05/06/2015: Notifica al Mud de la configuración actual del cliente (Encoding, número de columnas, etc)
    //                     Esta información se enviará siempre.
    _h = TN_IAC;
    _h += TN_SB;
    _h += GMCP;
    /*
     ...
     *    }
     */
    _h += string("Core.Supports.Configuration { ") +
          string("\"encoding\" : \"") +
          encoding.toLatin1().data() +
          string("\"") +
          string(",\"cols\" : ") +
          QString::number(mpHost->mpConsole->console->getColumns()).toLatin1().data()+
          string(",\"rows\" : ") +
          QString::number(mpHost->mpConsole->console->getRows()).toLatin1().data()+
          string("}");
    _h += TN_IAC;
    _h += TN_SE;
    ...
    // Ejecuta el evento que indica que se activa el GMCP.
    if(complete){
        TEvent me;
        me.mArgumentList.append( "sysGMCPenabledEvent" );
        me.mArgumentTypeList.append( ARGUMENT_TYPE_STRING );
        mpHost->raiseEvent( me );
    }
}
