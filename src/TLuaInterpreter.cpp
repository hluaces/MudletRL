...
int TLuaInterpreter::getLines( lua_State * L )
{
    ...
    for( int i=0; i<strList.size(); i++ )
    {
        ...
        // Zoilder 11/03/2015: Debe usarse UTF-8.
...
int TLuaInterpreter::feedTriggers( lua_State * L )
{
    ...
    if( lua_isstring( L, 2 ) )
    {
        luaWindowName = lua_tostring( L, 2 );
    }
    else
        luaWindowName = "main";
    ...
    if( name == "main" ){
        pHost->mpConsole->printOnDisplay( luaSendText );
    }
    else{
        mudlet::self()->printOnDisplay(pHost, name, luaSendText);
    }
...
/*
    ...
    if( lua_isstring( L, 1 ) )
    {
        // Zoilder 11/03/2015: Mudlet trabaja con Utf, pero en este punto es necesario simular como si la entrada procediera
        // del Mud, por lo que se convierte el texto del trigger simulado a latín, para que se lea correctamente.
        luaWindowName = lua_tostring( L, 1 );
        //luaWindowName = QString::fromUtf8(lua_tostring( L, 1 )).toLatin1().constData();
    }
    else
    {
        lua_pushstring( L, "feedTriggers: wrong argument type" );
...
int TLuaInterpreter::setWindowWrap( lua_State * L )
{
    ...
    if( name == "main" ){
        ...
        // Zoilder 29/03/2015: Actualiza el wrap del host (Para que así se actualice el wrap de la ventana de settings)
        pHost->mWrapAt = luaFrom;
    }
    else
...
int TLuaInterpreter::setWindowWrapIndent( lua_State * L )
{
    ...
    // Zoilder 29/03/2015: Si se establece el indent de la ventana principal, se actualiza el valor del host,
    //                     para que así aparezca actualizado en la ventana de Settings.
    if(name == "main")
        pHost->mWrapIndentCount = luaFrom;
...
int TLuaInterpreter::createLabel( lua_State *L )
{
    ...
    Host * pHost = TLuaInterpreter::luaInterpreterMap[L];
    QString name = QString::fromUtf8(luaSendText.c_str());
...
int TLuaInterpreter::hideUserWindow( lua_State *L )
{
    ...
    Host * pHost = TLuaInterpreter::luaInterpreterMap[L];
    // Zoilder 13/03/2015: Se crean los datos en UTF-8.
...
int TLuaInterpreter::sendGMCP( lua_State *L )
{
    string msg;
    Host * pHost = TLuaInterpreter::luaInterpreterMap[L];
    // Zoilder 13/06/2015: Limito el envío de GMCP a que esté activo en el cliente.
    if(!pHost->mEnableGMCP){
        lua_pushstring( L, "sendGMCP: Inactive GMCP protocol. Activate it in Settings." );
        lua_error( L );
        return 1;
    }
...
int TLuaInterpreter::tempTimer( lua_State *L )
{
    ...
    Host * pHost = TLuaInterpreter::luaInterpreterMap[L];
    TLuaInterpreter * pLuaInterpreter = pHost->getLuaInterpreter();
    // Zoilder 13/03/2015: LUA ahora se usa en UTF-8.
    QString _fun = QString::fromUtf8(luaFunction.c_str());
    int timerID = pLuaInterpreter->startTempTimer( luaTimeout, _fun );
    lua_pushnumber( L, timerID );
    return 1;
}
...
bool TLuaInterpreter::compileAndExecuteScript( QString & code )
{
    ...
    // Zoilder 14/03/2015: Ahora LUA va en UTF-8.
...
bool TLuaInterpreter::compile( QString & code, QString & errorMsg )
{
    ...
    // Zoilder 11/03/2015: Todo el texto se maneja en UTF por la parte de Mudlet, así que los scripts/triggers/etc. no son una excepción.
    //int error = luaL_dostring( L, code.toLatin1().data() );
    int error = luaL_dostring( L, code.toUtf8().data() );
    QString n;
    if( error != 0 )
...
void TLuaInterpreter::parseJSON( QString & key, QString & string_data, QString protocol )
{
    ...
    {
        // Zoilder 11/03/2015: Todo se debe de hacer en UTF-8.
    ...
    // Zoilder 11/03/2015: Se pasa el texto tal cual, que estará en formato UTF-8.
    //                      Reemplaza los caracteres problemáticos para el parseador.
    //lua_pushlstring(L, string_data.toLatin1().data(), string_data.size());
    lua_pushlstring(L, string_data.toUtf8().data(), string_data.toUtf8().size());
...
bool TLuaInterpreter::call( QString & function, QString & mName )
{
    ...
    // Zoilder 12/03/2015: Se pasa a utf8.
...
bool TLuaInterpreter::callEventHandler( QString & function, const TEvent & pE )
{
    if( function.isEmpty() )
        return false;
    lua_State * L = pGlobalLua;
    // Zoilder 12/03/2015: Se pasa a UTF-8.
    int error = luaL_dostring(L, QString("return " + function).toUtf8().data());
    if( error != 0 )
    {
        string e;
        if( lua_isstring( L, 1 ) )
    ...
    for( int i=0; i<pE.mArgumentList.size(); i++ )
    {
        ...
        {
            // Zoilder 11/03/2015:
            //  El texto se debe tratar siempre como Utf8.
...
void TLuaInterpreter::set_lua_string( const QString & varName, QString & varValue )
{
    ...
    // Zoilder 12/03/2015: Se pasan los datos en utf.
...
void TLuaInterpreter::initLuaGlobals()
{
    ...
    // Zoilder 29/03/2015: Funciones para obtener el número de columnas / filas de una consola.
    lua_register( pGlobalLua, "getColumns", TLuaInterpreter::getColumns);
    lua_register( pGlobalLua, "getRows", TLuaInterpreter::getRows);
    // Zoilder 05/06/2015: Función para permitir reactivar el GMCP desde la mudlib, para refrescar los datos GMCP, etc.
    lua_register( pGlobalLua, "resetGMCP", TLuaInterpreter::resetGMCP );
...
int TLuaInterpreter::startPermSubstringTrigger( QString & name, QString & parent, QStringList & regexList, QString & function )
{
    ...
    // Zoilder 29/03/2015: Función que devuelve el número de columnas de la consola indicada (o la main console)
int TLuaInterpreter::getColumns( lua_State * L )
{
    string luaWindowName="";
    if( lua_gettop( L ) == 0 )
    {
    ...
    else
    {
        if( ! lua_isstring( L, 1 ) )
        {
            lua_pushstring( L, "getColumns: wrong argument type" );
            lua_error( L );
            return 1;
        }
        else
    ...
    // Main Window.
    if(name == "main")
        c = pHost->mpConsole->console->getColumns();
    else
        c = mudlet::self()->getColumns(pHost, name);

    lua_pushnumber( L, c );
    return 1;
}

// Zoilder 29/03/2015: Función que devuelve el número de filas de la consola indicada (o la main console)
int TLuaInterpreter::getRows( lua_State * L )
{
    string luaWindowName="";
    if( lua_gettop( L ) == 0 )
    {
    ...
    else
    {
        if( ! lua_isstring( L, 1 ) )
        {
            lua_pushstring( L, "getRows: wrong argument type" );
            lua_error( L );
            return 1;
        }
        else
    ...
    // Main Window.
    if(name == "main")
        c = pHost->mpConsole->console->getRows();
    else
        c = mudlet::self()->getRows(pHost, name);
...
// Zoilder 05/06/2015: Función para permitir reactivar el GMCP desde la mudlib, para refrescar los datos GMCP, etc.
int TLuaInterpreter::resetGMCP(lua_State * L){
    Host * pHost = TLuaInterpreter::luaInterpreterMap[L];
    pHost->mTelnet.resetGMCP(true);
}
