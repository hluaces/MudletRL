...
bool TAlias::match( QString & toMatch )
{
    ...
    //const char *error;
    // Zoilder 15/03/2015: toLocal8Bit -> toUtf8.
    char * subject = (char *) malloc(strlen(toMatch.toUtf8().data())+1);
    strcpy( subject, toMatch.toUtf8().data() );
    ...
    for( i=0; i < rc; i++ )
    {
        ...
        // Zoilder 15/03/2015: fromLocal8Bit -> fromUtf8.
        captureList.push_back( QString::fromUtf8(match.c_str()).toUtf8().data() );
        posList.push_back( ovector[2*i] );
        if( mudlet::debugMode ) {TDebug(QColor(Qt::darkCyan),QColor(Qt::black))<<"Alias: capture group #"<<(i+1)<<" = ">>0; TDebug(QColor(Qt::darkMagenta),QColor(Qt::black))<<"<"<<QString::fromUtf8(match.c_str()).toUtf8().data()<<">\n">>0;}
    }
    ...
    for(;;)
    {
        ...
        for( i = 0; i < rc; i++ )
        {
            ...
            captureList.push_back( QString::fromUtf8(match.c_str()).toUtf8().data() );
            posList.push_back( ovector[2*i] );
            if( mudlet::debugMode ) {TDebug(QColor(Qt::darkCyan),QColor(Qt::black))<<"capture group #"<<(i+1)<<" = ">>0; TDebug(QColor(Qt::darkMagenta),QColor(Qt::black))<<"<"<<QString::fromUtf8(match.c_str()).toUtf8().data()<<">\n">>0;}
        }
    }
...
void TAlias::setRegexCode( QString code )
{
    ...
    const char *error;
    // Zoilder 15/03/2015: toLocal8Bit -> toUtf8.
    char * pattern = (char *) malloc( strlen( code.toUtf8().data() ) + 48 );
    strcpy( pattern, code.toUtf8().data() );
    int erroffset;
