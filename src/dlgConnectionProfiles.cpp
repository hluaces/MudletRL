...
void dlgConnectionProfiles::slot_item_clicked(QListWidgetItem *pItem)
{
    ...
    if( val.size() < 1 )
    {
        ...
        if( profile_name == "Slothmud")
            val = "slothmud.org";
        // Zoilder 18/03/2015: Reinos de leyenda MUD.
        if( profile_name == "Reinos de Leyenda")
            val = "rlmud.org";
    }
    ...
    if( val.size() < 1 )
    {
        ...
        // Zoilder 06/04/2015: Reinos de leyenda MUD.
        if( profile_name == "Reinos de Leyenda")
            val = "<center><a href='http://www.reinosdeleyenda.es/'>http://www.reinosdeleyenda.es/</a></center>";
    }
...
void dlgConnectionProfiles::fillout_form()
{
    ...
    // Zoilder 18/03/2015: Reinos de leyenda MUD.
    muds = "Reinos de Leyenda";
    pM = new QListWidgetItem( muds );
    pM->setFont(font);
    pM->setForeground(QColor(255,255,255));
    profiles_tree_widget->addItem( pM );
    mi = QIcon( QStringLiteral( ":/icons/rlmud_120_30.png" ) );
    pM->setIcon(mi);
    muds.clear();
    ...
    for( int i=0; i<mProfileList.size(); i++ )
    {
        ...
        if( mProfileList[i] == "3Kingdoms" )
            continue;
        // Zoilder 18/05/2015: Reinos de leyenda Mud.
        if( mProfileList[i] == "Reinos de Leyenda" )
            continue;
        QString sList;
