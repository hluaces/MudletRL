...
, mpScrollBar( 0 )
{
    ...
    if( ! mIsDebugConsole )
    {
        ...
        // Zoilder 17/03/2015: Se almacena la fuente usada.
        mDisplayFont = mpHost->mDisplayFont;
    }
    else
    {
...
void TTextEdit::slot_toggleTimeStamps()
{
    ...
    forceUpdate();
    update();
    // Zoilder 11/06/2015: Reenvía a la mudlib la información nueva de columnas.
    mpConsole->mpHost->mTelnet.resetGMCP(false);
}
...
void TTextEdit::updateScreenView()
{
    ...
    if( ! mIsDebugConsole && ! mIsMiniConsole )
    {
        ...
        {
            /* Zoilder 25/03/2015: Se usará el ancho calculado, no 100, pues de lo contrario
             *                     no siempre realizará el wrap adecuado al escribir textos en
             *                     la ventana.
...
void TTextEdit::drawForeground( QPainter & painter, const QRect & r )
{
    ...
    {
        // Zoilder 21/03/2015: La primera vez que se conecta al Mud, la primera vez que el buffer
        //                     supera el tamaño de la pantalla, dejará una parte sin dibujar, en lugar
        //                     de subir la pantalla para arriba y así dar cabida al nuevo texto.
        //                     Por lo tanto, se fuerza esta situación.
        //                     En nuestro Mud esto sucede al dibujar el ascii de entrada.
        //                     Si dicho ascii hace que se supere el número de líneas máxima que caben en la
        //                     pantalla, dejará las primeras líneas de la pantalla sin dibujar hasta que
        //                     se refresque.
        //                     Una vez que refresque, ya no volverá a pasar hasta abrir una nueva ventana.
        //mScrollVector = lineOffset - mLastRenderBottom;
        if(mLastRenderBottom!=0)
            mScrollVector = lineOffset - mLastRenderBottom;
        else
    ...
    for( int i=from; i<=y2; i++ )
    {
        ...
        // Zoilder 16/03/2015: Variable para controlar la posición inicial de cada texto a dibujar.
        int x = x1;
        for( int i2=x1; i2<lineLength; )
        {
            QString text;
            // Zoilder 16/03/2015: Se dibuja el texto controlando el ancho de cada carácter,
            //                     en lugar de usar uno fijo, pues de lo contrario,
            //                     con ciertos caracteres de idiomas que sean más anchos
            //                     que la W (el que se usa como base), hará que los textos se pisen.
            int ancho = 0;
            ...
            else
            {
                ...
                // Zoilder 16/03/2015: Incrementa el ancho del primer carácter.
                ancho += QFontMetrics( mDisplayFont ).width( text.at(0) );
                while( i2+delta+timeOffset < lineLength )
                {
                    if( mpBuffer->buffer[i+lineOffset][i2+delta-timeOffset] == f )
                    {
                        // Zoilder 16/03/2015: Incrementa el ancho con el del carácter actual.
                        ancho += QFontMetrics( mDisplayFont ).width( mpBuffer->lineBuffer[i+lineOffset].at(i2+delta-timeOffset) );
                        text.append( mpBuffer->lineBuffer[i+lineOffset].at(i2+delta-timeOffset) );
                        delta++;
                    }
                    else
                ...
                // Zoilder 16/03/2015: Se pasan la posición x y el ancho según se ha calculado más arriba.
                textRect = QRect( x,/*mFontWidth * i2,*/
                                  mFontHeight * i,
                                  ancho,/*mFontWidth * delta,*/
                                  mFontHeight );
                ...
                i2+=delta;
                // Zoilder 16/03/2015: Actualiza la posición x del siguiente texto que se dibuje.
                x+=ancho;
            }
        }
    }
...
void TTextEdit::clear(){
    // Reinicia las posiciones actuales.
    mLastRenderBottom = 0;
    mpBuffer->mCursorY=0;
}

    // Zoilder 29/03/2015: Función que devuelve el número de columnas(Caracteres) que caben en el campo de texto.
    //                     Si se están mostrando los timestamps, le resta 13 columnas.
int TTextEdit::getColumns(){
    return mScreenWidth - ( mShowTimeStamps ? 13 : 0 );
}

    // Zoilder 29/03/2015: Función que devuelve el número de filas que caben en el campo de texto.
int TTextEdit::getRows(){
