...
, actionReplaySpeedUp( 0 )
{
    ...
    // Zoilder 17/07/2015: Fuente para la barra de herramientas.
    mpMainToolBar->setFont(QFont("Segoe UI", 8, QFont::Courier));
    addToolBarBreak();
...
void mudlet::addConsoleForNewHost( Host * pH )
{
    ...
    // Zoilder 04/04/2015: Modificado para que se compilen todos los scripts y actualice el editordialog.
    //                     Debe hacerse todo en este punto para que no aparezcan los scripts inicialmente como
    //                     erróneos y no haya errores a la hora de refrescar la consola (si no,
    //                     inicialmente habrá errores por no estar aún disponibles los eventos de resize
    //                     que definan los scripts.
    mpCurrentActiveHost->mLuaInterpreter.loadGlobal();
    LuaInterface * lI = mpCurrentActiveHost->getLuaInterface();
    lI->getVars( true );
    mpCurrentActiveHost->getScriptUnit()->compileAll();
    // Zoilder 04/04/2015: Esto estaba encima de pH->getActionUnit()->updateToolbar().
    ...
    int x = mpCurrentActiveHost->mpConsole->width();
    int y = mpCurrentActiveHost->mpConsole->height();
    /* Zoilder 04/04/2015: Desactivado, pues se realiza automáticamente a la hora de ejecutar 'refresh' sobre la consola.
    QSize s = QSize(x,y);
    QResizeEvent event(s, s);
    QApplication::sendEvent( mpCurrentActiveHost->mpConsole, &event);
    */
...
bool mudlet::createMiniConsole( Host * pHost, QString & name, int x, int y, int width, int height )
{
    ...
    if( ! dockWindowConsoleMap.contains( name ) )
    {
        ...
        if( pC )
        {
            ...
            // Zoilder: 21/05/2015: La miniconsola gestionará el MXP igual que la consola principal.
            pC->buffer.mMXP = pHost->mpConsole->buffer.mMXP;
            return true;
        }
    }
    else
...
bool mudlet::clearWindow( Host * pHost, QString & name )
{
    ...
    if( dockWindowConsoleMap.contains( name ) )
    {
        ...
        // Zoilder 29/03/2015: Limpia también el campo de texto (Reiniciará la posición del cursor, etc.)
        dockWindowConsoleMap[name]->console->clear();
...
bool mudlet::setLabelClickCallback( Host * pHost, QString & name, QString & func, const TEvent & pA )
{
    ...
    if( labelMap.contains( name ) )
    {
        //qDebug()<<"SetLabelClickCallback:" << name;
...
{
    /* Zoilder 07/04/2015: Comento esto, pues provoca que cierre consolas, pero deje las pestañas cargadas
     *                      si se cancela el cierre de alguna de ellas, lo que provoca un pete en la aplicación
     *                      en cuanto se pulse sobre alguna pestaña.
     *                      Ahora se cierra cada consola y oculta su pestaña, hasta que se cancele el cierre de alguna.
     *                      Se hace todo más abajo.
     *
    ...
    foreach( TConsole * pC, mConsoleMap )
    {
        QString name = pC->mpHost->getName();

        if( pC->mpHost->getName() != "default_host" )
        {
            // close console
            // Zoilder 07/04/2015: Si cancela el cierre, se corta el evento en este punto.
            if(!pC->close()){
                mIsGoingDown = false;
                event->ignore();
                return;
            ...
            // Zoilder 07/04/2015: Esto se hacía antes de cerrar, ahora se hará una vez que se cierre.
            // close script-editor
            ...
            // Zoilder 07/04/2015: Cierra también la pestaña, de lo contrario, si en alguna de las conexiones
            //                      se cancela el cierre del cliente, petará al entrar en una de las pestañas vacías (o
            //                      si dicha pestaña es la activa)
            if( mTabMap.contains( name ) )
            {
                int tabIndexFound = -1;
                for(int i=0; i<mpTabBar->count(); ++i) {
                    if(name == mpTabBar->tabText(i)) {
                        tabIndexFound = i;
                        break;
                    }
                }

                mpTabBar->removeTab( tabIndexFound );
                mConsoleMap.remove( pC->mpHost );
                mTabMap.remove( name );
...
void mudlet::slot_mapper()
{
    ...
    {
        // Zoilder 07/04/2015: Se está descargando el mapa.
        if(pHost->mpMap->mpMapper->mpProgressDialog &&
            pHost->mpMap->mpMapper->mpProgressDialog->isVisible()){
            QMessageBox msgBox;
            msgBox.setText("Map is downloading.");
            msgBox.exec();
            return;
        }
        ...
        // Basta con ocultar / mostrar el widget, no es necesario hacer lo mismo con el mapa en sí, o lo abrirá a tamaño mínimo
        // si el widget no estaba enlazado a algún margen de la ventana.
    ...
    // Zoilder 07/04/2015: Se controla si puede restaurar el mapa. Cuando no hay mapa y se indica
    //                     que lo descargue, no mostrará el widget (Hasta que acabe la descarga)
    bool canRestore = true;
    if( pHost->mpMap->mpRoomDB->getRoomIDList().size() < 1 )
    {
        canRestore = pHost->mpMap->restore("");
    ...
    // Zoilder 07/04/2015: No ha podido restaurar, oculta el widget.
    //                     Se hace así para asegurar que añade correctamente el widget.
    if(!canRestore){
        pHost->mpMap->mpMapper->parentWidget()->setVisible( false );
        pHost->mpMap->mpMapper->setVisible( false );
        return;
    }
...
void mudlet::slot_connection_dlg_finnished( QString profile, int historyVersion )
{
    ...
    /*Zoilder 04/04/2015: Estas tareas se hacen en addConsoleForNewHost, para
                          que a la hora de refrescar la consola inicialmente, todos los scripts
                          que definan el evento resize estén disponibles, si no, pitará error..
...
// Zoilder 29/03/2015: Función para escribir textos en miniconsolas,
//                     aplicando formato (Como si se escribieran en la ventana principal)
void mudlet::printOnDisplay( Host * pHost, QString & name, std::string & text)
{
    QMap<QString, TConsole *> & dockWindowConsoleMap = mHostConsoleMap[pHost];
    if( dockWindowConsoleMap.contains( name ) )
    {
        dockWindowConsoleMap[name]->printOnDisplay(text);
        dockWindowConsoleMap[name]->console->showNewLines();
        dockWindowConsoleMap[name]->console2->showNewLines();
    }
}

    // Zoilder 29/03/2015: Devuelve el número de columnas de la consola.
int mudlet::getColumns( Host * pHost, QString & name )
{
    QMap<QString, TConsole *> & dockWindowConsoleMap = mHostConsoleMap[pHost];
    if( dockWindowConsoleMap.contains( name ) )
    {
        return dockWindowConsoleMap[name]->console->getColumns();
    }
    else
        TDebug(QColor(Qt::white),QColor(Qt::red))<<"ERROR: window doesn't exit\n" >> 0;

    return 0;
}

    // Zoilder 29/03/2015: Devuelve el número de filas de la consola.
int mudlet::getRows( Host * pHost, QString & name )
{
    QMap<QString, TConsole *> & dockWindowConsoleMap = mHostConsoleMap[pHost];
    if( dockWindowConsoleMap.contains( name ) )
    {
        return dockWindowConsoleMap[name]->console->getRows();
    }
    else
        TDebug(QColor(Qt::white),QColor(Qt::red))<<"ERROR: window doesn't exit\n" >> 0;
