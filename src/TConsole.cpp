...
, mSearchQuery("")
// Zoilder 07/04/2015: Inicializa el nuevo campo que indica que debe mostrar el mapa.
,mShowMap(false)
{
...
//    mDisplayFont.setLetterSpacing( QFont::AbsoluteSpacing, 0 );
    ...
    mpCommandLine->setFont(mpHost->mCommandLineFont);
...
{
    // Zoilder 11/03/2015:
    //  El texto proviene del mud siempre en UTF8.
    //std::string key = buf;
    //QString sheet = sh.c_str();
    std::string key = buf.c_str();
    QString sheet = QString::fromUtf8(sh.c_str());
    /*
        qDebug() << "Set Label Style Sheet";
        qDebug() << "Latin: " << QString::fromLatin1(buf.c_str());
        qDebug() << "UTF: " << QString::fromUtf8(buf.c_str());
        qDebug() << "Normal: " << buf.c_str();
    */
    ...
    if( profile_name != "default_host" )
    {
        ...
        if( mpHost->mFORCE_SAVE_ON_EXIT )
        {
            ...
            if( mpHost->mpMap->mpRoomDB->size() > 0 )
            {
                ...
                // Zoilder 07/04/2015: Si el mapa estuviera visible, lo oculta.
                if ( mpHost->mpMap->mpMapper &&
                     mpHost->mpMap->mpMapper->isVisible() &&
                     mpHost->mpMap->mpMapper->parentWidget()->inherits("QDockWidget") ){
                        mpHost->mpMap->mpMapper->parentWidget()->setVisible( false );
                        mpHost->mpMap->mpMapper->setVisible( false );
                }
            }
            event->accept();
            return;
...
void TConsole::slot_toggleLogging()
{
    ...
    if( mLogToLogFile )
    {
        ...
        // Zoilder 21/03/2015: Se logueará en UTF-8, para estar acorde al encoding del Mudlet.
        mLogStream.setCodec(mpHost->mTelnet.encoding.toLatin1().data());
...
void TConsole::slot_toggleReplayRecording()
{
    ...
    {
        mReplayFile.close();
        // Zoilder 21/03/2015: Corrección para que muestra el nombre del fichero correcto (usaba el fichero de log, no el de replay)
...
void TConsole::showEvent( QShowEvent * event )
{
    if( ! mIsDebugConsole && ! mIsSubConsole )
    {
        if( mpHost )
        {
            ...
            // Zoilder 07/04/2015: Si se ocultó el mapa al cambiar de pestaña, lo hace visible de nuevo.
            if ( mShowMap == true ){
                mpHost->mpMap->mpMapper->parentWidget()->setVisible( true );
                mShowMap = false;
            }
        }
    }
...
void TConsole::printCommand( QString & msg )
{
    ...
    else
    {
        ...
        if( lineBeforeNewContent >= 0 )
        {
            ...
            /* Zoilder 25/03/2015: Inactivo esto, pues de lo contrario, al dibujar un comando por pantalla, si
             *                      este comando se usa después de que se muestre por pantalla un texto
             *                      que no provoque prompt (si lo último que se ve por pantalla no es el prompt),
             *                      hace que no se realice el wrap de dicho comando.
            ...
            {*/
                ...
                // Zoilder 29/03/2015: Añade al comienzo del comando un salto de línea.
                msg.push_front('\n');
                buffer.insertInLine( P, msg, format );
                /* Zoilder 25/03/2015: Usa el valor de Wrap establecido en la configuración,
                 *                     para que quede cuadrado también el comando a dibujar por pantalla.
                 */
                int down = buffer.wrapLine( lineBeforeNewContent,
                                            mpHost->mWrapAt/*mpHost->mScreenWidth*/,
...
{
    // Zoilder 11/03/2015:
    //  El texto proveniente de la Mudlib será siempre en UTF8.
    //std::string key = name.toLatin1().data();
    std::string key = name.toUtf8().data();
    if( mSubConsoleMap.find( key ) == mSubConsoleMap.end() )
    {
