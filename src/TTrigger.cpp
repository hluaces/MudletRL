        ...
        if( propertyList[i] == REGEX_PERL )
        {
            ...
            if (re == 0)
            {
                if( mudlet::debugMode )
                {
                    ...
                    TDebug(QColor(Qt::red),QColor(Qt::gray))<<QString::fromUtf8(pattern).toUtf8().data()<<"\n">>0;
                }
                setError( QString( "Pattern '" )+QString::fromUtf8(pattern).toUtf8().data()+QString( "' failed to compile. Correct the pattern.") );
                state = false;
